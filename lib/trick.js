...
const _addCard = (trick, card, username) => {
	if (_addCardToPosition(trick, "first", card, username)) return true;
	if (_addCardToPosition(trick, "second", card, username)) return true;
	if (_addCardToPosition(trick, "third", card, username)) return true;
	return false;
};
...
class Trick {
	...
	throw(card, username) {
		_validate(card, username);
		if (_addCard(trick, card, username)) return this;
